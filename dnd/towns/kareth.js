...
const APP_VERSION = "2026-05-21-kareth-town-sheet-v1";
console.log("ANG Town Sheet JS:", APP_VERSION);
...
/**
 * URL style:
 *   /dnd/towns/town.html?kareth
 * Optional cache-bust allowed:
 *   /dnd/towns/town.html?kareth&v=3
 */
function getSlugFromUrl() {
  const q = window.location.search;
  if (!q || q === "?") return "";
  const raw = q.slice(1);              // remove "?"
  const first = raw.split("&")[0];     // take "kareth" from "kareth&v=3"
...
function readElementValue(el) {
  if (el.type === "number") return safeNum(el.value, 0);
...
async function loadTown() {
  if (!slug) {
    ...
    setStatus("No town specified. Use: town.html?kareth");
    return;
  }
  ...
  if (error) {
    console.warn("Town not found or load failed. Loading defaults.", error);
    applyTownToUI(DEFAULT_TOWN);
    renderLastSaved(null);
    setStatus("Not found in DB. Defaults loaded; create row or save after insert policy is ready.");
  ...
  const town = data.data_json || DEFAULT_TOWN;
  applyTownToUI({ ...deepClone(DEFAULT_TOWN), ...town });
...
async function saveTown() {
  ...
  if (error) {
    console.error(error);
    setStatus("Save failed (check console). Does the towns row exist?");
    return;
  }
